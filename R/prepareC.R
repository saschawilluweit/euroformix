...
#' @param kit kit object generated by getKit() or NULL if degradation is not desired
...
#loc=locs[23]
 ...
 if(!is.null(kit)) { 
   if(length(kit)>1) { #Updated: check that it is not only NA returned
    for(loc in locs) { #for each markers
     slist[[loc]] <- numeric()
     subkit <- subset(kit,toupper(kit$Marker)==loc) #control on dyer
